...
class TT(MenuWithField):
    ...
    def next(self):
        self.step += 1
        if self.step >= len(self.menu_ui_settings["textlines"]):
            self.message = "load"
            return
        textline = self.menu_ui_settings["textlines"][self.step]
    ...
    def findtile(self):
        ...
        for cat, item in self.examplelist.items():
            for i in item:
                nd[i["nm"]] = cat
        name = self.find(nd, self.menu_ui_settings["findmenu_text"])
        if name is None:
            return
