...
class Menu:
    ...
    def recaption(self):
        pg.display.set_caption(f"OGSCULEDITOR+: {self.menu} | {tag} | {self.custom_info}")
...
class MenuWithField(Menu):
    ...
    def recaption(self):
        pg.display.set_caption(f"OGSCULEDITOR+: {self.menu} | {tag} | {os.path.relpath(self.data['path'], application_path) if os.path.exists(self.data['path']) else 'Unsaved level'} | {self.custom_info}")
